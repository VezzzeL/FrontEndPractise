...
function logIn(username, password) {
  return new Promise(function (resolve, reject) {
    const userSelected = usersDataBase.filter(
      (word) => word.username === username
    );
    const passwordSelected = userSelected.filter(
      (word) => word.password === password
    );
    if (userSelected.length && passwordSelected.length) {
      resolve(true);
    } else {
      reject("Try again! Your username (or/and) your password could be wrong!");
    }
  });
}
...
function setPlaylist(qtySongs) {
  return new Promise(function (resolve, reject) {
    if (qtySongs <= songsDataBase.length) {
      const songsSelected = songsDataBase.filter(
        (song, index) => index < qtySongs
      );
